...
> ⚡ **异步生成（v1.2.7 起）**：日报的 LLM 分析与渲染耗时较长，工具调用会**快速返回**"已开始生成"，AI 先正常回复用户；生成在后台进行，**完成后自动把日报图片发到群里**，并触发 AI 补一句"日报已生成"（与之前的对话自然衔接，不重复发送日报）。期间不会阻塞 AI 回复，也不会触发工具调用超时。
...
### v1.2.9 (2026-08-10) — 连接自愈修复 🐛

**🐛 修复**
- 🔧 **数据库连接自愈**：修复插件热重载/卸载后数据库连接被关闭导致消息收集持续报错（`ValueError: no active connection`）。`close()` 后连接置空，所有数据库操作前自动检查并重连，无需重启 KiraAI
...
**✨ 新增功能**
- ⚡ **自然语言触发改异步生成**：LLM 工具调用 `generate_daily_report` 改为**快速返回**——AI 先回复用户，日报的 LLM 分析与渲染在**后台任务**中进行，完成后自动把日报发到群里，并触发 AI 补一句"日报已生成"（与对话自然衔接）
- 🔄 **重复请求保护**：同一会话日报正在生成时再次请求，会提示"正在生成中"，不会重复触发
- 🧹 **任务清理**：插件卸载时自动取消后台生成任务

**🐛 修复**
- 🕐 **工具调用不再超时**：此前日报生成耗时可能超过工具调用超时上限导致失败；异步化后工具立即返回，彻底规避

### v1.2.6 (2026-07-10) — 稳定性修复 🐛

**🐛 修复**
- 🔧 **修复头像下载失败问题**：解决 Windows 环境下 `aiohttp` 异步 DNS 解析失败导致的头像无法下载（`Could not contact DNS servers`），强制使用 `ThreadedResolver` 同步解析，兼容性大幅提升
- 🔧 **修复 `asyncio` 未导入错误**：`avatar.py` 中 `await asyncio.sleep()` 缺少导入导致插件初始化失败

**🔧 优化**
- 📸 **头像下载增加重试机制**：网络偶发波动时自动重试 1 次，提高成功率
- 📝 **增加详细错误日志**：头像下载失败时输出具体原因（HTTP 状态码 / 异常信息），方便排查
- 🌐 **添加请求头**：为头像下载请求添加 `User-Agent` 和 `Referer`，模拟浏览器行为，降低被服务器拒绝的风险
- 🔒 **忽略 SSL 验证**：解决嵌入式 Python 环境证书缺失导致的下载失败
