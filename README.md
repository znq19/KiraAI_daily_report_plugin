...
- **👤 活跃用户**：直接从数据库统计发言最多的用户及发言数（零 Token 消耗）
- **💡 金句识别**：自动筛选群聊中的精彩发言，LLM 动态评选"最佳金句"
...
- **📋 手动触发**：`/日报` 命令或自然语言调用
- **🔄 增量分析**：滑动窗口分批处理 + 最近 N 批次限制，新话题快速上位
...
| 参与统计的最近批次数量 | 5 个 | 值越小新话题上位越快，建议 3-8 |

> 💡 **增量模式 + 最近批次限制**：只统计最近的 N 个增量批次，旧话题随批次滑出自动淘汰，新话题持续出现即可上位，实现真正的"滚动更新"。
...
| 🔥 热门话题 | LLM 提取 + AI 吐槽，按出现频次排序 |
| 👤 活跃用户 | 从原始消息精确统计，发言最多的 N 位用户 |
| 💡 今日金句 | 精选发言 + LLM 动态评选最佳金句 + 理由 |
| 🤖 AI 锐评 | 带人设风格的一句点评 |
...
- 检查 `参与统计的最近批次数量` 配置（默认 5）
- 值越小新话题上位越快，建议 3-8
- 旧话题只有在最近 N 个批次中持续出现才能保留，否则自动淘汰
...
- 🔄 **最近批次数量限制**：只统计最近 N 个增量批次，新话题快速上位
- 📋 **配置分组**：schema.json 重构为 10 个分组，清晰易用

**🔧 优化改进**
- 💬 **副标题（header_comment）**：增量模式下由 LLM 统一生成，不再显示默认值
- ⭐ **最佳金句**：由 LLM 从候选池中动态评选，不再固定取第一条
